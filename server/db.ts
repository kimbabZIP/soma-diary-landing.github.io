...
import { InsertUser, users, waitlist, type InsertWaitlist } from "../drizzle/schema";
...
export async function getWaitlistByReferralCode(code: string) {
  const db = await getDb();
  if (!db) return undefined;
  const result = await db.select().from(waitlist).where(eq(waitlist.referralCode, code)).limit(1);
  return result.length > 0 ? result[0] : undefined;
}
...
export async function getWaitlistRank(id: number): Promise<number> {
  const db = await getDb();
  if (!db) return 0;
  const result = await db
    .select({ count: sql<number>`count(*)` })
    .from(waitlist)
    .where(sql`${waitlist.id} <= ${id}`);
  return Number(result[0]?.count ?? 0);
}
